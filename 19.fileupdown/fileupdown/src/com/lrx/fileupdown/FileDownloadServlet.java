...
//        IOUtils.copy(resourceAsStream, outputStream);
        ...
        //1. 获取下载资源的名字
        String downloadFile = request.getParameter("name");
        System.out.println("name= " + downloadFile);

        //2.给http响应头设置content-type，就是文件的MIME
        ServletContext servletContext = request.getServletContext();
        String downloadPath = "/download/";
        String downloadFileFullPath = downloadPath + downloadFile;
        String mimeType = servletContext.getMimeType(downloadFileFullPath);
        System.out.println("下载资源的MIME类型= " + mimeType);
        response.setContentType(mimeType);


        //3. 给http响应，设置响应头 Content-Disposition
        //   这里考虑的细节比较多，比如不同的浏览器写法不一样，考虑编码
        //   ff 是 文件名中文需要 base64, 而 ie/chrome 是 URL编码
        //   这里我们不需要同学们记住，只需知道原理
        //   老韩解读
        ...
        if (request.getHeader("User-Agent").contains("Firefox")) {
            // 火狐 Base64编码
            response.setHeader("Content-Disposition", "attachment; filename==?UTF-8?B?" +
                    new BASE64Encoder().encode(downloadFile.getBytes("UTF-8")) + "?=");
        } else {
            // 其他(主流ie/chrome)使用URL编码操作
            response.setHeader("Content-Disposition", "attachment; filename=" +
                    URLEncoder.encode(downloadFile, "UTF-8"));
        }
